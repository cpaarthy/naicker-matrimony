...
export default function Dashboard({ onNavigate, showToast }) {
  ...
  return (
    <div>
      <h2 className="serif" style={{ fontSize: 19, marginBottom: 14 }}>Dashboard</h2>
      ...
      {!profile && (
        <div style={{ textAlign: "center", padding: "30px 16px", background: colors.card, borderRadius: 14, border: `1px solid ${colors.cardBorder}`, marginBottom: 16 }}>
          <div style={{ color: colors.text, fontWeight: 600, marginBottom: 6 }}>No profile yet</div>
          <div style={{ fontSize: 13, color: colors.textFaint, marginBottom: 14 }}>Complete your profile to start receiving interest requests.</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <DashCard icon={Heart} title="Interest Requests" badge={pendingIncoming > 0 ? pendingIncoming : null} onClick={() => onNavigate("requests")} colors={colors} />
        <DashCard icon={ShieldCheck} title="Favourites" onClick={() => onNavigate("favourites")} colors={colors} />
        <DashCard icon={Mail} title="Contact Us" onClick={() => onNavigate("contact")} colors={colors} />
      </div>
    </div>
  );
}
