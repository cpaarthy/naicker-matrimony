...
export default function AdminDashboard({ adminPin, onLogout, onNavigate, setSelectedProfileId, showToast }) {
  ...
  async function handleFullBackup() {
    ...
    downloadJson(`naicker-matrimony-backup-${new Date().toISOString().split('T')[0]}.json`, data);
    setBackingUp(false);
  ...
  return (
    <div>
      ...
      {tab === "log" && <ActivityLogTab colors={colors} showToast={showToast} adminPin={adminPin} />}
    </div>
  );
}
...
function ActivityLogTab({ colors, showToast, adminPin }) {
  ...
  async function loadUsers() {
    const { data } = await fetchAllProfiles(adminPin);
...
function PoruthamTab({ colors, profiles, showToast }) {
  ...
  async function checkPorutham() {
    ...
    if (!calculated) {
      showToast("Missing or unrecognized star/rasi data for one or both profiles");
      setResult(null);
      setLoading(false);
      return;
    }
      ...
      {result && (
        <div style={{ background: colors.card, border: `1px solid ${colors.cardBorder}`, borderRadius: 12, padding: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, fontSize: 12 }}>
            {result.calculated.poruthams.map((m, i) => (
              <div key={i} style={{ padding: "4px 8px", borderRadius: 4, background: m.matched ? colors.approvedBg : colors.rejectedBg, color: m.matched ? colors.approvedText : colors.rejectedText }}>
                {m.label}: {m.matched ? "Match" : "No match"}
              </div>
            ))}
          </div>
        </div>
